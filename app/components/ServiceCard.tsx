...
export default function ServiceCard({ title, description, imageSrc }: Props) {
  ...
  const imgSrc = imageSrc ? imageSrc : `data:image/svg+xml;utf8,${encodeURIComponent(placeholderSvg)}`;

  return (
    <article className="group rounded-2xl border border-slate-200 bg-white overflow-hidden transition hover:-translate-y-1 hover:border-[#3ea4c9]/40 hover:shadow-[0_18px_35px_rgba(9,57,95,0.16)] flex">
